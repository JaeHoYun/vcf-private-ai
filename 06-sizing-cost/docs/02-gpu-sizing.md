...
예) 80GB GPU, `gpu_memory_utilization` 0.9 → 가용 약 72 GiB. 8B FP16(가중치 약 16 GiB) + 활성화·오버헤드 약 4 GiB → 가용 KV 약 52 GiB. 요청당 KV 약 0.5 GiB(4K, 2.3 계산 예제) → 최대 동시성 약 100 `[A1 가정치]`. 이 상한이 곧 "이 한 장으로 몇 요청을 동시에 받느냐"입니다.
