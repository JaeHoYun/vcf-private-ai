...
| 일반 워커 노드 풀 | 1개 이상 | CPU 기반 워크로드와 시스템 애드온용 |
...
| 데몬셋 컴포넌트 | 역할 |
|---|---|
| nvidia-driver-daemonset | GPU 드라이버 설치와 관리 |
...
> 산정 팁: GPU 노드의 "워크로드 가용 RAM" = 노드 총 RAM − OS, kubelet 예약 − GPU Operator 데몬셋 합계 − 시스템 파드 여유. 실제 예약량은 컴포넌트 버전과 설정별로 상이하므로 4.6 실측으로 확정하세요.
...
| 5. 일반 워커 노드 | 시스템과 CPU 워크로드 | 예: 3 노드 (HA) |
| 6. 컨트롤 플레인 | HA | 3 노드 |
| 7. 노드 합계(클러스터당) | 합산 | 3 + 3 + 12 = **18 노드** |
| 8. 클러스터 수 | 격리와 한도 반영 | 팀 5개 격리 → 5 클러스터 |
...
---
[← 이전: 03 컴퓨트와 메모리 사이징](03-compute-memory-sizing.md) | [목차](../README.md) | [다음: 05 스토리지와 네트워크 용량 사이징 →](05-storage-network-sizing.md)
