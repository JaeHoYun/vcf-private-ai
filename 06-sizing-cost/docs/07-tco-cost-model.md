...
이 문서는 시리즈 ⑥(`vcf-private-ai-sizing-cost`)의 마무리 문서로, 02~05에서 도출한 사이징 결과(GPU 수, 노드 수, 스토리지 용량)를 총소유비용(TCO) 항목으로 환산하는 모델과 워크시트를 제시합니다. 이 문서의 목적은 "얼마인지"를 단정하는 것이 아니라, "무엇을 어떤 구조로 더해야 하는지"를 빠짐없이 보여주는 데 있습니다.
...
라이선스는 TCO에서 가장 오해가 많은 항목입니다. 핵심 사실관계는 다음과 같으며, 이 구조가 산정의 출발점입니다. (라이선스 구조의 상세는 ① 라이선스 절 [vcf-private-ai-guide](../../01-infra/README.md)를 참조하세요.)
