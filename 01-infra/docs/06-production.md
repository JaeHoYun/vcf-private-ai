...
자동 스케일링: `Min Replicas`(HA 보장) / `Max Replicas`(상한) / `Target GPU Utilization`(예 70%). 야간과 주말은 Min으로 축소해 비용 최적화.

PAIS 3.0은 데이터 평면의 스케일 단위를 나눴습니다. Ingress를 모델 엔드포인트와 독립적으로 스케일할 수 있고, 커스텀 모델 게이트웨이를 주입할 수 있으며, Agent Builder와 Data Indexing을 개별로 켜고 끌 수 있습니다. 서빙만 쓰는 배포라면 두 모듈을 끄고 그만큼의 컨트롤 플레인 자원을 아낄 수 있습니다. 지연과 세션 임계 기반의 모델 자동 스케일링(Model Autoscaling)은 2026-08 Explore에서 발표됐으나 아직 릴리스 노트에 없으므로, 레플리카 수는 여전히 수동 설정으로 계획합니다([문서 00 0.7.4절](00-whats-new.md)).
...
|------|----------|---------------|
| **버전 관리** | 모델, 가중치, 설정, 프롬프트를 버전으로 고정(재현 가능) | Model Gallery(Harbor) 태그, 설정은 GitOps 코드화(6.3절) |
| **평가(게이트)** | 승격 전 정확도, 관련성, 안전성, 지연 기준 충족 검증 | Agent Playground, 오프라인 평가셋, 통과 기준을 승격 게이트로 |
| **승격(Promotion)** | DEV → STAGING → PROD로 단계 승격, 트래픽 점진 전환 | 네임스페이스 분리(6.4절), Replica 점진 전환과 카나리 |
| **롤백(Rollback)** | 지표 저하 시 직전 안정 버전으로 복귀 | 이전 버전 태그 유지 → 빠른 재배포, 자동(임계치)와 수동(승인) 병행 |
| **재학습 트리거** | 데이터 드리프트, 성능 저하, 신규 데이터 발생 시 재학습 | 관측성 지표(6.8절) 임계치 → 파이프라인 트리거, 결과는 다시 버전 관리로 |

> 위는 일반 MLOps 모델 라이프사이클 원칙입니다(모델 레지스트리의 stage 전환, 평가 게이트, 자동/수동 롤백과 드리프트 기반 재학습). 운영 시 **각 전환마다 평가 기준과 책임자 승인을 문서로 명시**하고, 롤백을 위해 직전 버전 아티팩트를 항상 보존하세요. 부수효과가 큰 PROD 승격과 롤백은 변경관리 절차와 연동하는 것을 권장합니다.

> **에이전트 포함 시:** 모델뿐 아니라 KB 버전, 프롬프트, **MCP 도구/권한 정책**도 함께 버전과 롤백 대상입니다(6.3절 DR 자산, [문서 05 5.5절](05-agents-mcp.md#55-거버넌스-가장-중요)).
...
| 모델 수준 가시성 | Endpoint별 요청과 오류율, Replica 상태 |

### LLM 트레이싱 (OpenTelemetry)

OTel Collector로 에이전트 요청을 단계별(RAG 검색 → MCP 도구 호출 → LLM 추론)로 추적합니다 → [문서 05 5.6절](05-agents-mcp.md#56-에이전트-관측성-llm-트레이싱).
...
흐름은 **미러 호스트에서 `vcf pais amt pull`로 아티팩트를 끌어와 오프라인 반입 → 내부망에서 `vcf pais amt push`로 내부 Harbor에 적재 → 모델 갤러리(`vcf pais models`) 적재 → Supervisor에 PAIS 설치**입니다. 용량 경계는 플랫폼 Harbor 프로젝트 **20GB**(PAIS 아티팩트)이며 모델 갤러리와 NIM 이미지는 별도 산정합니다(6.9.2절). Artifact Mirroring Tool이 서명과 다이제스트를 자동 검증한다는 공식 근거는 없으므로, 반입 아티팩트의 무결성과 공급망 점검은 **운영자가 수행하는 별도 절차**입니다(6.9.2절 보안 가이드 딥링크).
...
| 무결성과 공급망 | 운영자가 수행하는 점검 → ⑤ 보안 가이드 딥링크 |
| 반입과 store 용량 | 플랫폼 Harbor 프로젝트 20GB(PAIS 아티팩트) + 모델/NIM 별도 → ⑥ 사이징 가이드 딥링크 |
