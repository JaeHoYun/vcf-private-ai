...
기준 버전: VCF 9.1.1 / DSM 9.1.1 / PAIS 3.0. 상세는 [01 버전 호환 매트릭스](01-version-compatibility.md)를 참조하시기 바랍니다. 본 문서의 UI 경로와 절차는 공식 문서와 공개 랩 가이드를 기반으로 하며, 릴리스에 따라 화면 명칭이 일부 달라질 수 있으므로 공식 문서를 함께 확인하시기 바랍니다.
...
**DSM 9.1.1 배포 전 확인(2026-09)** — Avi Load Balancer와 NSX 네트워킹을 함께 쓰는 클러스터는 VCF를 9.1.0 이상으로 올리기 전에 DSM을 9.1.1로 먼저 올려야 데이터베이스 다운타임을 피합니다. PostgreSQL 12와 13 인스턴스는 9.1.1에서 지원이 끊기므로 배포 전에 14 이상으로 올립니다([01 버전 호환 매트릭스](01-version-compatibility.md)).
...
PostgreSQL 버전은 용도에 따라 고릅니다. PAIS Data Indexing이 붙는 인스턴스는 PAIS 3.0 릴리스 노트가 검증한 16.8 조합을 유지하고, 앱이 직접 질의하는 벡터 검색 전용 인스턴스에서만 DSM 9.1.1이 새로 지원하는 17.10이나 18.4를 검토합니다. 18은 9.1.1에서 처음 지원되므로 pgvector 확장 동작을 PoC로 확인한 뒤 채택하십시오.
...
| max_parallel_maintenance_workers | CPU 코어 - 1 | 병렬 빌드(06 운영 보안 주의 참조) |

인덱스가 메모리에 상주하지 못하면 성능이 10–100배 저하될 수 있으므로, 데이터 적재 후 실측 인덱스 크기로 사이징을 재검증합니다. 규모별 권장 구성은 03 아키텍처와 08 PoC를 참조합니다.
