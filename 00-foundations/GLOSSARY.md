...
| 학습 (Training) | 데이터로 모델의 가중치를 만들어 내는 과정. 대규모 GPU가 필요한 일회성과 배치 작업. |
...
| MCP (Model Context Protocol) | 모델이 외부 도구와 데이터에 표준 방식으로 연결되는 인터페이스. |
| 파라미터 (Parameter) | 모델이 학습으로 얻은 가중치 수. 모델 크기와 GPU 메모리 요구량을 좌우한다. |
| 양자화 (Quantization) | 모델 가중치의 정밀도를 낮춰 메모리와 연산을 줄이는 기법. |
...
| HBM (High Bandwidth Memory) | 최신 AI GPU의 VRAM으로 쓰이는 고대역폭 메모리. 칩을 적층해 GDDR보다 용량당 대역폭이 크다. |
| 메모리 대역폭 (Memory Bandwidth) | 초당 메모리에서 코어로 옮기는 데이터량(GB/s). 토큰 생성 속도를 좌우하는 추론의 숨은 병목. |
| KV 캐시 (KV Cache) | 생성 중인 각 요청의 문맥을 담는 가변 GPU 메모리. 동시 요청과 컨텍스트가 길수록 커져 동시 처리 수를 제한한다. |
| Tensor Core | 행렬 곱셈에 특화된 GPU 전용 연산 유닛. 낮은 정밀도(FP16, FP8 등)를 빠르게 처리한다. |
...
| CUDA | GPU에 일반 연산을 시키기 위한 NVIDIA의 플랫폼과 런타임. 상위 AI 소프트웨어 대부분이 이 위에서 동작한다. |
| cuDNN | CUDA 위에서 딥러닝 연산을 최적화해 둔 라이브러리. |
| NCCL | 여러 GPU 간 고속 통신을 담당하는 라이브러리. 멀티 GPU 학습과 추론의 토대. |
...
| NIM (NVIDIA Inference Microservices) | NVIDIA의 추론 컨테이너 런타임/모델 포맷. NVAIE 라이선스가 필요한 엔터프라이즈 콘텐츠. |
| vGPU Manager (VIB) | 한 GPU를 여러 VM이 나눠 쓰는 vGPU를 위해 ESXi 호스트에 설치하는 NVIDIA 컴포넌트. NVAIE 라이선스 계정으로만 내려받는다. |
