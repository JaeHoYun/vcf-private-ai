...
| ⑦ | [07-design](07-design/README.md) | 통합 설계 | 16개 설계 결정, 레퍼런스 블루프린트(소, 중, 대), 설계 결정 기록 카탈로그 (전 계층 종합) |
...
이 시리즈는 Private AI **플랫폼**을 구축, 운영, 보호, 설계하는 ①–⑦입니다. 그 플랫폼 **위에 앱과 에이전트 서비스를 올리는** 일은 시리즈의 번호 챕터가 아니라 별도의 최상위 가이드에서 다룹니다. 플랫폼(이 시리즈)과 그 위 실행 계층(앱과 에이전트)을 분리해, 프로필의 **AX(전략) → Private AI(플랫폼) → 앱과 에이전트 서비스(실행)** 3단계 구성과 맞춥니다.
