...
|---|---|---|---|
| VI Admin | 인프라와 워크로드 도메인 운영 | GPU-Accelerated Workload Domain 구성, 네임스페이스 생성 | 모델 가중치 직접 열람, KB 콘텐츠 변경 |
| MLOps | 모델 배포와 런타임 운영 | 모델 갤러리 배포, Model Runtime 스케일링, API 게이트웨이 정책 | IdP/SSO 설정 변경, 감사 로그 삭제 |
...
PAIS는 OpenAI API와 호환되는 인터페이스를 제공하며, API 게이트웨이가 모델 엔드포인트로 들어오는 요청의 인증과 인가와 부하 분산을 담당합니다([Private AI Services New in VCF 9.0, Broadcom VCF Blog](https://blogs.vmware.com/cloud-foundation/2025/06/19/private-ai-services-new-in-vmware-private-ai-foundation-with-nvidia-in-vcf-9-0/)).
...
## 3.4 에이전트와 MCP 접근통제: 도구 등록과 승인 게이트
...
| 시크릿 유형 | 사용처 | 회전 기준 |
|---|---|---|
| 모델 레지스트리 자격증명 | 모델 갤러리와 런타임 배포 | 정기 + 인력 변동 시 |
| 벡터 DB / DB 자격증명 | 데이터 인덱싱과 검색(RAG) | 정기 + 노출 의심 시 즉시 |
...
> 확인 필요: PAIS가 내장 비밀 저장소를 제공하는지, 외부 KMS/Vault 연동을 전제로 하는지는 배포 토폴로지에 따라 다를 수 있어 공식 배포 문서로 별도 확인이 필요합니다.
...
| 4 | RBAC 최소 권한 | 앱 개발자 계정으로 MCP 등록 승인과 인프라 변경 시도 | 권한 부족으로 거부 |
| 5 | 직무 분리 | 감사자 계정으로 쓰기와 삭제 시도 | 모두 거부, 읽기만 허용 |
...
| 7 | 도구 스코프 추적 | 도구 갤러리에서 도구별 사용 에이전트와 승인 이력 확인 | 추적 체인 누락 0건 |
| 8 | KB 권한 일치 | 원본 데이터 소스 권한 없는 사용자가 에이전트로 해당 KB 질의 | 우회 열람 불가 |
| 9 | mTLS 내부 통신 | 마이크로서비스 간 통신 캡처로 상호 TLS 적용 확인 | 평문과 단방향 TLS 0건 |
| 10 | 시크릿 회전 | 시크릿 인벤토리의 만료와 회전 이력 점검 | 만료 없는 시크릿 0건(예외 승인 제외) |
