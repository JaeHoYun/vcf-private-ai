...
| 구분 | 보호 자산 | 손상 시 영향 |
|---|---|---|
| 데이터 | 학습/RAG 코퍼스, 임베딩 벡터, 프롬프트와 응답 로그 | 기밀 유출, 추론 데이터 역추적 |
...
|---|---|---|---|---|
| 1 | 데이터 인입 | 데이터와 모델 포이즈닝, 오염 코퍼스 주입 | LLM04 / ATLAS Poison Training Data | 05 |
| 2 | 인덱싱(임베딩) | 벡터와 임베딩 약점, 임베딩 역전, 교차테넌트 누수 | LLM08 / ATLAS RAG Poisoning | 05 |
...
| 6 | 에이전트와 MCP | 과도한 권한(Excessive Agency), 도구 오남용 | LLM06 / ATLAS Multi-agent trust exploitation | 03, 06 |
| 7 | 공급망 | 모델, 이미지, 라이브러리 변조 | LLM03 / ATLAS AI Supply Chain Compromise | 04 |
| 8 | 앱과 전체 | 오정보(환각) 전파, 감사 부재 | LLM09 / — | 06, 07 |
...
다음 경계를 넘는 데이터와 제어 흐름은 모두 인증, 인가, 검사 대상입니다(상세 격리 설계는 [02-network-tenant-isolation.md](02-network-tenant-isolation.md)).

- 외부 사용자 ↔ 앱/게이트웨이: 인증과 입력 검증 경계. 간접 인젝션 차단 지점.
...
PAIF는 에어갭 배치를 지원하며, Artifact Mirroring Tool(PAIS 2.1 신규)을 통해 GPU 모델 엔드포인트와 에이전트를 포함한 전 기능을 외부 인터넷 없이 운용할 수 있습니다([Broadcom: Secure Private AI Part 2](https://blogs.vmware.com/cloud-foundation/2026/04/30/guide-to-secure-private-ai-with-broadcom-part-2/)).
...
|---|---|---|---|
| 하이퍼바이저와 GPU 격리 | 제공 | 패치와 구성 적용 | — |
| 네트워크 세그멘테이션 | 엔진 제공(vDefend/NSX) | 정책 설계와 적용 | 워크로드 태깅 |
| ID, RBAC | 인증 연동 제공 | 역할과 최소권한 설계 | 서비스계정 위생 |
| 모델과 이미지 공급망 | 레지스트리와 서명 기능 | 미러링과 승인 게이트 | 모델 출처 검증 |
| 데이터 거버넌스 | 암호화와 벡터DB 제공 | 키와 보존정책 | 코퍼스 분류와 마스킹 |
| 앱 가드레일 | Agent Builder 프레임 | 가드레일 정책 표준 | 프롬프트와 출력 검증 |
| 감사와 컴플라이언스 | 로그 소스 제공 | SIEM, 증빙 체계 | 앱 레벨 이벤트 발생 |
...
| LLM02 민감정보 노출 | 응답과 로그를 통한 기밀 유출 | 05 | 06, 07 |
| LLM03 공급망 | 모델, 이미지, 의존성 변조 | 04 | 03 |
| LLM04 데이터와 모델 포이즈닝 | 오염 데이터로 무결성 훼손 | 05 | 04 |
| LLM05 부적절 출력 처리 | 출력의 하류 시스템 주입 | 06 | — |
| LLM06 과도한 권한 | 에이전트/MCP 권한 남용 | 03 | 06 |
| LLM07 시스템 프롬프트 유출 | 지시문과 비밀 노출 | 06 | 07 |
| LLM08 벡터와 임베딩 약점 | 인덱스 누수, 역전, 오염 | 05 | 02 |
| LLM09 오정보 | 환각과 근거 없는 출력 | 06 | 07 |
| LLM10 무한 소비 | 자원 고갈과 비용 폭증 | 06 | 02, 07 |
