...
- **경로 B(Model Endpoint)**: 03에서 앱이 만든 컨텍스트를 완성 모델에 직접 보냅니다. 검색과 조립을 앱이 통제합니다.

두 경로의 차이는 한마디로 **"검색을 누가 통제하는가"** 입니다. 경로 B는 앱이 검색과 조립을 직접 짜 모델에 넘기고, 경로 A는 그 오케스트레이션을 플랫폼(Agent)에 맡깁니다.
...
경로 A의 검색은 파이프라인에 고정된 단계가 아니라 **에이전트가 호출 여부와 검색어를 스스로 정하는 행위**입니다(에이전틱 검색, 4.3절). 단순 질의는 검색을 건너뛰기도 합니다.
...
- **`model`이 어디서 도는지 확인(PAIS 3.0부터)**: 같은 `base_url` 뒤에 로컬 모델, 다른 인스턴스의 공유 모델, Google Gemini 같은 원격 클라우드 모델이 함께 놓일 수 있습니다. 위 코드의 `assembled_context_and_question`에는 03에서 조립한 검색 청크가 그대로 들어가므로, `model`이 원격 모델이면 그 청크가 사외로 나갑니다. 사내 문서 Q&A는 사내 모델을 기본으로 두고, 원격 모델은 [⑤ 05 5.6절](../../05-security/docs/05-data-governance.md)의 허용 목록에 든 지식베이스에만 씁니다.

## 4.3 에이전트 호출 (경로 A)

Agent를 쓰면 검색이 호출 안에 포함됩니다. 앱은 Knowledge Base에 연결된 Agent에 질문을 던지고, Agent가 검색→조립→생성을 수행해 답과 근거를 반환합니다. Agent 선언과 호출 엔드포인트와 요청/응답 스키마는 ③과 [PAIS 공식 API 레퍼런스](https://developer.broadcom.com/xapis/vmware-private-ai-service-api/latest/)를 따릅니다.
...
| 모델 위치(3.0부터) | 사내 모델 기본. 원격 클라우드 모델은 반출 정책이 허용한 지식베이스에만(⑤ 05 5.6절) |

---
[← 이전: 03 검색과 컨텍스트 조립](03-retrieval-context.md) | [목차](../README.md) | [다음: 05 앱 통합 패턴 →](05-app-integration.md)
