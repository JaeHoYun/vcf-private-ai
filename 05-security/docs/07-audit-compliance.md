...
7.2.2까지는 플랫폼 위에서 도는 AI의 관측입니다. 그런데 관측 대상에 오르지 않은 사용 — 개인 계정의 공개 챗봇, AI 기능이 켜진 SaaS, 개인 결제 API 키, 부서가 자체로 붙인 MCP 서버와 에이전트 — 는 위 표 어디에도 나타나지 않습니다. AX 방법론은 이를 섀도 AI(shadow AI, 조직이 존재를 모르는 채 쓰이는 AI)로 정의하고 발견 채널 셋(기술 관측, 재무 기록, 사람)과 양성화 절차를 정했습니다([AX 방법론 10 10.4절](https://github.com/JaeHoYun/enterprise-ax-methodology/blob/main/docs/10-governance.md)). 이 절은 그중 기술 관측 채널을 이 플랫폼이 이미 갖고 있는 신호로 구체화합니다. 재무 기록과 자진 신고는 플랫폼 밖의 채널이므로 방법론 쪽을 따릅니다.
