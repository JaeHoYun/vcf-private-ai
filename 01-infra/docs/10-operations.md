...
- 현재 버전 인벤토리 + 변경 영향 분석 → 문서 00 0.6절 (9.0.x→9.1 체크리스트), 문서 00 0.9절 (9.1→9.1.1 / PAIS 2.1→3.0 체크리스트) 연계
- 업그레이드 전 백업 선행 (VCF 구성, pgvector/DSM, PAIS 설정) → 문서 06 6.3절
...
컴포넌트별 롤백 가능 범위가 다르므로, **업그레이드 전 백업과 스냅샷이 사실상의 롤백 수단**입니다.

- 모델 아티팩트는 Harbor 태그로, PAIS 설정은 GitOps로 보존하면 재구성 경로가 확보됩니다([문서 06 6.3절](06-production.md)).
- 직전 안정 버전 아티팩트를 항상 보존합니다.
- 부수효과가 큰 PROD 업그레이드와 롤백은 변경관리 절차와 연동하는 것을 권장합니다.
...
- 배포 후 검증 체크리스트 재사용 → 문서 06 6.11절
```

### 10.1.7 인접 영역 위임

- 보안 패치 정책과 CVE 대응 → [⑤ 보안과 거버넌스 가이드](../../05-security/README.md)
- 업그레이드에 따른 용량 변동과 재산정 → [⑥ 사이징, 용량, 비용 가이드](../../06-sizing-cost/README.md)
- 9.0.x → 9.1 한눈 변경 체크리스트 → [문서 00](00-whats-new.md) 0.6절
...
모델 갤러리(모델 가중치 아티팩트)는 Artifact Mirroring Tool과 별개로 `vcf pais models` 명령으로 적재하고 조회합니다 ([VCF CLI 명령 레퍼런스](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-consumption/latest/consumer-interfaces-in-vcf/installing-and-using-vcf-cli-v9/command-reference2/pais2.html)).
...
증상 식별 → **계층 좁히기**(인프라 / Kubernetes / PAIS / 앱) → 로그와 이벤트 확인 → 알려진 이슈 대조 → 조치와 검증.

| 계층 | 1차 확인 |
|------|----------|
| Supervisor / PAIS | `kubectl get paisconfiguration`(READY=True), `kubectl get pods` + Pod 이벤트 |
| GPU | GPU Operator Pod 상태, 드라이버와 MIG 설정, GPU 메트릭 대시보드([문서 06 6.8절](06-production.md)) |
...
| vGPU 드라이버가 Unlicensed로 표시 | 라이선스 무효, 만료, 형식 오류 | 라이선스가 JWT 형식인지 검증(jwt.io로 디코드해 만료와 node server URL 확인) 후 재적용 |
| GPU Pod 스케줄 실패 | vGPU 호스트 드라이버와 SR-IOV 미구성, GPU 쿼터 소진 | 각 ESX의 SR-IOV(BIOS), vGPU 드라이버 확인, 네임스페이스 `nvidia.com/gpu` 쿼터 확인 |

### 10.2.3 모델 엔드포인트

| 증상 | 가능 원인 | 조치 |
|------|----------|------|
| 업그레이드 후 Endpoint 재배포 실패 | 메모리 부족 (PAIS 2.1 알려진 이슈) | 리소스 여유 확보 후 재배포, 적정 모델과 양자화 검토 |
| 첫 응답이 느림 (Cold Start) | 새 Replica의 모델 로딩 지연 | Min Replicas ≥ 1, 워밍업 요청, 이미지 사전 Pull ([문서 06 6.5절](06-production.md)) |
| 간헐 오류, 일부 Replica 장애 | GPU/Pod 장애 | Health Check가 자동 제외와 교체([문서 06 6.2절](06-production.md)), GPU 메트릭 점검 |
...
> **적용 전 확인:** 위 Workaround는 공식 릴리스 노트와 문서 기준이며 버전별로 해결되고 변경될 수 있습니다. 적용 전 [PAIS 릴리스 노트](https://techdocs.broadcom.com/us/en/vmware-cis/private-ai/foundation-with-nvidia/9-0/private-ai-release-notes/vmware-private-ai-services-release-notes.html)로 재확인하시기 바랍니다.
...
| AI 자산 | 모델(Harbor), 벡터(pgvector/DSM), PAIS 설정(GitOps), 앱 데이터 | [문서 06 6.3절](06-production.md) |
...
- 복원 후 헬스, 일관성 검증(10.1.6절 검증 재사용)
- RTO/RPO 실측값 측정 → 문서 06 6.3절 DR 패턴과 대조
...
| GPU 건전성 | GPU 사용률과 메모리 압력 | 임계 초과 시간 비율 |

> SLO 수치는 워크로드, 모델, 하드웨어에 좌우되므로 **기준선(baseline)을 실측해 설정**합니다([문서 06 6.6절](06-production.md) 사이징, 6.8절 관측 연계). 위 값은 형식 예시이며 실제 목표값이 아닙니다.
...
- **1차 대응 = 트러블슈팅 런북(10.2절)** 으로 증상 → 진단 → 조치. 미해결 시 에스컬레이션합니다.
- **에스컬레이션 경로**: 1차(운영자) → 2차(플랫폼팀) → 벤더 지원(Broadcom). 보안 사고는 [⑤ 보안과 거버넌스 가이드](../../05-security/README.md) 절차로 분기.
- **사후**: 인시던트 기록과 포스트모템, 반복 이슈는 알람과 런북에 반영(피드백 루프).

> **적용 전 확인:** VCF Operations의 Alert/Symptom/Notification 프레임워크와 9.1 AI, GPU 지표(TTFT, 토큰 처리량, GPU 사용률)는 공식 문서 기준이나, 정확한 알람 정의와 지표 명칭은 적용 전 [알람 정의 모범사례 문서](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/infrastructure-operations/configuring-alerts-and-actions/defining-alerts-best-practices.html)로 확인하시기 바랍니다.
...
| 리밸런스와 resync | **Automatic Rebalance**(용량 불균형이 임계 초과 시, 최대 30분 대기 후 시작). resync는 유지보수 모드, 리밸런스, 정책 변경, 장애 복구 시 발생하며 그동안 용량이 "operations usage"로 표시 | **ESA + 10G**에서는 resync 트래픽이 Adaptive Resync 20% 목표를 넘어 VM IO에 영향 줄 수 있음 → 유지보수 창, 대역 고려 |
...
| 스토리지 지연 → 모델 콜드스타트 | datastore 지연과 처리량이 모델 로딩 속도를 좌우 | 콜드스타트 증상은 10.2.3절, 스토리지 정책(SPBM) 점검 |

### 10.5.3 인접 위임 (경계 한 줄)

- **구성과 설계**(NSX 토폴로지, vSAN 설계, GPUDirect RDMA 구성) → [문서 02](02-architecture.md), [문서 09](09-deployment-scenarios.md)
...
분기:  복구 훈련(10.3.2절), 업그레이드 계획 점검(10.1절), 멀티테넌트 격리 회귀
```

### 10.6.3 운영 책임 경계 (한 줄 정리)

- **① 이 문서**: 플랫폼/인프라 Day-2 운영(업그레이드, 장애, 백업복구, 인증서, SLO, 네트워크, 스토리지).
- **PAIS 플랫폼**: 일상 운영의 상당 부분을 자동화와 관측으로 처리([문서 06 6.12절](06-production.md)). 운영자는 HA/DR, 거버넌스, 라이프사이클에 집중.
- **⑤ 보안 / ⑥ 용량**: 보안 운영과 용량과 비용은 각 가이드로 위임.
