...
- **절차 골격:** [문서 02 §2.6 구축 Phase 개요](02-architecture.md)가 기준 문서입니다(여기서 반복하지 않음). 요약하면 **VCF 9.1.x → PAIF(GPU) Workload Domain → Harbor/DSM → PAIS 3.0 → 카탈로그** 순입니다. PAIS 3.0부터는 배포 시점에 Agent Builder와 Data Indexing 모듈을 개별로 켜고 끌 수 있으므로, 모델 서빙만 필요한 그린필드라면 두 모듈을 끄고 시작해 필요할 때 켜는 편이 초기 자원과 운영 범위를 줄입니다.
