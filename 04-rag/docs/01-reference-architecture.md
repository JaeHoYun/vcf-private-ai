...
LLM은 학습 시점까지의 일반 지식만 압니다. **사내 정책, 계약, 기술 문서**는 모릅니다. 파인튜닝으로 주입할 수도 있으나 비용과 갱신 주기 문제가 큽니다. RAG(Retrieval-Augmented Generation)는 다른 접근입니다 — 질문이 들어오면 **사내 문서에서 관련 조각을 검색해 프롬프트에 끼워 넣고**, 모델이 그 근거를 바탕으로 답하게 합니다. 모델은 그대로 두고, 지식은 외부(벡터 DB)에 둡니다.

장점은 셋입니다. 첫째, 문서를 갱신하면 즉시 반영된다(재학습 불필요). 둘째, 답변에 **출처**를 붙일 수 있다. 셋째, 데이터가 모델 가중치에 박히지 않아 접근 통제가 쉽다.
...
| 벡터 저장과 검색 | DSM PostgreSQL + pgvector | ② |
...
앱이 임베딩 API, pgvector, 완성 API를 직접 호출하며 파이프라인을 손수 조립합니다. PAIS의 **OpenAI 호환 API**(임베딩과 완성)와 ②의 pgvector를 그대로 씁니다.
...
02–03은 인덱싱과 검색의 **원리와 의사결정**(경로 무관 공통 지식), 04–05는 **PAIS API/Agent로의 실제 연결**, 06–07은 **품질과 운영**입니다. 각 문서 끝의 "핵심 결정" 표만 모아 읽어도 설계 체크리스트가 됩니다.

---
[목차](../README.md) | [다음: 02 데이터 인입과 인덱싱 →](02-ingestion-indexing.md)
