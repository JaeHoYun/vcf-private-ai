...
| KV 캐시 (KV Cache) | 생성 중인 각 요청의 문맥을 담는 가변 GPU 메모리. 동시 요청·컨텍스트가 길수록 커져 동시 처리 수를 제한한다. |
...
| NGC (NVIDIA GPU Cloud) | NVIDIA의 컨테이너·모델·프레임워크 카탈로그. 엔터프라이즈(인증) 콘텐츠·NIM은 NVAIE 라이선스가 필요하다. |
