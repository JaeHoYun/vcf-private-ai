...
기준 버전: VCF 9.1.1 / DSM 9.1.1 / PAIS 3.0. 상세는 [01 버전 호환 매트릭스](01-version-compatibility.md)를 참조하시기 바랍니다. DSM이 라이프사이클을 자동화하므로, 본 문서는 DSM 자동화 동작과 pgvector 워크로드 특화 운영을 함께 다룹니다.
...
DSM은 배포 시 자동 백업을 구성합니다. PostgreSQL 기본값은 주 1회 전체 백업, 일 단위 백업, WAL(Write-Ahead Log, 쓰기 전 로그)은 5분 또는 16MB 중 먼저 도달하는 시점마다 전송하며, 모든 DB에 대해 PITR(Point-In-Time Recovery, 특정 시점 복구)이 제공됩니다.
...
피크 대응(명절과 세일)에는 일시적으로 Scale Up 후 종료 시 원복하는 방식이 유효합니다.

### 패치/업그레이드

DSM이 어플라이언스, 게스트 OS, Kubernetes, DB 엔진 업데이트를 통합 번들로 제공합니다. DBA는 다운로드와 스테이징만 하면 유지보수 윈도(통상 토요일 밤부터 일요일 아침)에 롤링 적용됩니다. 메이저/마이너 버전 업그레이드 모두 지원되며, 메이저 업그레이드 시 확장(extension) 호환성을 사전 확인합니다.
...
PAIS는 Artifact Mirroring Tool을 통한 폐쇄망 환경을 지원합니다(2.1부터). VI Admin은 폐쇄망 내에서 NVIDIA GPU 기반 모델 엔드포인트와 에이전트를 포함한 Private AI 기능을 설치하고 운영할 수 있습니다. 방산, 금융, 공공 등 데이터 반출이 불가한 환경에서 RAG 전체 파이프라인을 외부 연결 없이 구성할 수 있습니다.

운영 시 모델과 컨테이너 아티팩트를 Artifact Mirroring Tool로 미러링하고, 갱신 주기에 맞춰 미러를 동기화합니다.
