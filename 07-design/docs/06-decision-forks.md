...
| D13 | AI 게이트웨이 계층 | PAIS 내장 + 앱 보호 vs 오픈소스 AI 게이트웨이 vs 기존 API 관리 플랫폼 확장 vs 결합 | 소수 팀, 차지백 불필요 → 내장 + 앱 보호 / 여러 사업부의 토큰 예산과 모델 혼용 → 오픈소스 지금 / API 관리 플랫폼이 정본 → 확장 / 대기업 → 결합. 어느 쪽이든 OpenAI 호환 계약과 모델 별칭으로 전환 비용을 낮춤 | [04 4.2절](04-network-storage-availability.md), [③ 05 5.7절](../../03-serving-api/docs/05-auth-and-gateway.md) |
