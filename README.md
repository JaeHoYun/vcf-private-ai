...
→ **[어디에 쓰나 — 성과가 나는 일, 실패하는 일](https://github.com/JaeHoYun/vcf-private-ai-apps/blob/main/docs/02-use-cases.md)**
...
| ⑦ | [07-design](07-design/README.md) | 통합 설계 | 14개 설계 결정, 레퍼런스 블루프린트(소, 중, 대), 설계 결정 기록 카탈로그 (전 계층 종합) |
...
- **설계를 종합하고 결정을 내린다** → ⑦ (14개 설계 결정과 블루프린트, ①–⑥로 딥링크)
...
이 시리즈는 Private AI **플랫폼**을 구축, 운영, 보호, 설계하는 ①–⑦입니다. 그 플랫폼 **위에 앱과 에이전트 서비스를 올리는** 일은 시리즈의 번호 챕터가 아니라 별도의 최상위 가이드에서 다룹니다. 플랫폼(이 시리즈)과 그 위 실행 계층(앱과 에이전트)을 분리해, 프로필의 **AX(전략) → Private AI(인프라) → 앱과 에이전트(실행)** 3단계 구성과 맞춥니다.

- [VCF Private AI 앱과 에이전트 서비스 가이드](https://github.com/JaeHoYun/vcf-private-ai-apps) — PAIS 3.0 위에서 LLM 앱, RAG 앱, 배치 파이프라인, 에이전트 서비스를 기획, 설계, 구축, 검증, 운영하는 실행 계층 가이드. Agent Builder, MCP, Model Runtime 구현을 포함하며 서빙(③), RAG(④)로 딥링크.
