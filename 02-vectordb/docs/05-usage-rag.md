...
| 미지원 문서 형식 | 미지원(커스텀 경로) | 위 형식 목록 밖. 한글 워드프로세서 형식(HWP, HWPX)도 목록에 없음 |
| 문서보안(DRM)으로 암호화된 문서 | 미지원(격리 존에서 복호화 전처리 뒤 S3 호환 스테이징을 커넥터로 읽거나 커스텀 경로) | 커넥터는 평문만 읽음. 지식베이스 안의 문서 단위 사용자별 권한 필터는 공식 문서에서 확인되지 않으므로 사용자 집단 단위로 지식베이스를 분리([④ 02 2.1.2절](../../04-rag/docs/02-ingestion-indexing.md), [앱 가이드 06](https://github.com/JaeHoYun/vcf-private-ai-apps/blob/main/docs/06-data-onboarding.md)) |
