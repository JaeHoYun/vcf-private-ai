...
| 데이터셋/문서 코퍼스 | vSAN 데이터스토어 | 원문 크기 × 보존 정책 | 파인튜닝([02 2.10절](02-gpu-sizing.md#210-학습과-파인튜닝-gpu-메모리))/RAG 소스 |
| 벡터 인덱스 | DSM pgvector(②) | 차원 × 벡터 수 × 인덱스 오버헤드 | 5.4에서 별도 산정 |
| 로그와 관측 데이터 | vSAN 데이터스토어 | 요청량 × 보존 기간 | 추론 로그와 메트릭 |
...
핵심은 **버전과 Revision 누적**입니다. Harbor는 모델과 NVIDIA 추론 컨테이너의 레지스트리로 동작하며, 보안팀이 스캔, 승인, RBAC로 관리하는 구조입니다([Broadcom TechDocs, "Storing ML Models in VMware Private AI Foundation"](https://techdocs.broadcom.com/us/en/vmware-cis/private-ai/foundation-with-nvidia/9-0/private-ai-foundation-9-x/what-is-private-ai-services/storing-ml-models-in-vmware-private-ai-foundation.html)). 동일 모델의 여러 버전을 보존하면 용량은 버전 수에 비례해 증가하므로, **보존 정책**(몇 개 버전을 남길지)을 먼저 정해야 합니다.

| 모델 규모(FP16 가정) | 가중치 1버전 | 3버전 보존 시 | 비고 |
|---|---|---|---|
| 7–8B | 약 14–16GB | 약 42–48GB | 어림, 실측 필요 |
| 70B | 약 140GB | 약 420GB | 어림, 실측 필요 |
| 405B | 약 810GB | 약 2.4TB | 확인 필요(정밀도와 샤딩 영향 큼) |

정밀도, 토크나이저, 세이프텐서 샤딩 방식에 따라 실제 디스크 점유는 달라지므로 위 값은 어림이며 **레지스트리 실측이 필요**합니다.

### 데이터셋과 로그

문서 코퍼스는 원문 용량에 비례하되, 청크/전처리 사본이 추가로 쌓이는 점을 감안합니다(원문 대비 1.5–2배 여유는 어림, 실측 필요). RAG용 벡터 DB까지 합친 저장 용량은 Broadcom VCF 9.1 설계 문서가 **원문 문서 용량의 5–10배**를 확보하도록 권장합니다([가속기 설계 PAIF-ACC-RCMD-010](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-library/private-ai-compute-detailed-design(1)/accelerator-detailed-design.html)). 5.2의 구성요소 합산과 이 비율을 둘 다 계산해 차이가 크면 벡터 수나 원문 용량 가정을 다시 봅니다(5.5). 추론 로그와 관측 데이터는 "초당 요청 × 요청당 로그 크기 × 보존 일수"로 산정하며 보존 정책에 가장 민감합니다.

---

## 5.2 벡터 인덱스 용량 — DSM pgvector 산정 (② 연계)

벡터 인덱스 용량은 [② 벡터 DB](../../02-vectordb/README.md)와 연계해 별도로 산정합니다. 본 절은 용량 환산식만 다루고, 인덱스 튜닝과 운영은 ②에 위임합니다.

### 벡터 수는 색인 범위로 정한다

벡터 수는 용량 계산의 출발 입력이지만, 임직원 수로 문서량이나 벡터 수를 정할 수 있는 공개 통계는 없습니다. 같은 조직도 **무엇을 색인하느냐**에 따라 100배 이상 차이가 나므로 색인 범위를 먼저 정합니다.

| 색인 범위 | 문서 수 규모 | 벡터(청크) 수 계산 범위 | 참고 |
|---|---|---|---|
| 부서나 업무 단위로 고른 문서(규정, 매뉴얼, FAQ, 기술 문서) | 약 1만 문서 | 약 12만–28만 | 한 서비스 조직의 사내 지식 검색 공개 사례가 약 1만 1천 문서 |
| 전사 차원에서 고른 지식베이스 | 약 10만 문서 | 약 120만–280만 | 대형 금융사와 컨설팅사의 사내 지식 도우미 공개 사례가 각각 약 10만 문서 |
| 전사 저장소 광범위 색인(파일 서버, 협업 저장소 대부분) | 필터 후 수십만–수백만 문서 | 수백만–수천만 이상 | 원천 저장소는 기업당 수천만 파일 규모이며 상당 부분이 중복, 노후 데이터 |

- 벡터 수 계산 가정: 문서당 약 6,300토큰(웹 PDF 약 4억 7,500만 건의 평균, [FinePDFs](https://infoq.com/news/2025/09/finepdfs/)), 청크 300–512토큰, 겹침 0–25%. 사내 오피스 문서는 분포가 다를 수 있으므로 자사 표본으로 바꿉니다.
- **원문 용량(GB)으로 벡터 수를 추정하지 않습니다.** 파일 용량 대비 추출 텍스트의 비율이 형식에 따라 크게 달라, 이미지 위주 PDF는 텍스트가 파일 크기의 1% 미만인 경우도 있습니다([Azure AI Search 청킹 예제](https://learn.microsoft.com/en-us/azure/search/vector-search-how-to-chunk-documents) 파일로 계산). 원문 용량은 원문 저장 용량 산정에만 씁니다.
- 구하는 법: (1) 유스케이스에 필요한 저장소만 고른다 (2) 저장소 메타데이터로 확장자별 파일 수와 용량, 최종 수정일을 집계하고 보존 기간, 최신 판, 중복 제거 기준으로 대상을 줄인다 (3) 형식별(PDF 텍스트형, PDF 스캔형, DOCX, PPTX, XLSX, HWP 등)로 수십–수백 파일을 표본으로 뽑는다 (4) 실제로 쓸 파서와 OCR로 텍스트를 추출하고 서빙할 임베딩 모델의 토크나이저로 토큰을 센다 (5) 형식별 파일 수 × 형식별 평균 토큰 ÷ (청크 크기 − 겹침)으로 벡터 수를 구한다. 표와 이미지 설명을 별도 청크로 만들면 그만큼 더한다.
...
> HNSW 인덱스를 1.5–3배 범위로 가정하면 합계는 위 값의 ±50% 이상 흔들립니다. 차원을 절반으로 줄이거나 float16(pgvector 0.7.0 이상, 인덱스 메모리 절반) 또는 양자화를 적용하면 크게 절감됩니다([Supabase, "What's new in pgvector v0.7.0"](https://supabase.com/blog/pgvector-0-7-0)). 따라서 위 표는 **상한 어림으로만** 사용하고 ②의 운영 지침에 따라 실측하세요.

또한 HNSW는 검색 성능을 위해 인덱스를 RAM에 상주시키는 것이 권장되므로, 위 용량은 **디스크뿐 아니라 메모리 사이징과도 직결**됩니다(메모리 산정은 [② 벡터 DB 가이드](../../02-vectordb/README.md)와 [03 컴퓨트와 메모리 사이징](03-compute-memory-sizing.md)으로 위임).
...
| 벡터 검색(pgvector) | 인덱스 RAM 상주 | 메모리와 지연 | 디스크 IOPS보다 RAM 상주 여부가 지배적 |
| 로깅/관측 | 지속 순차 쓰기 | 쓰기 대역폭 | 보존 정책에 따라 누적 |

콜드스타트 시간은 "가중치 용량 ÷ 유효 순차 대역폭"으로 1차 추정하되, 레지스트리 압축 해제, 네트워크 경유, GPU 적재 단계가 합산되므로 단순 나눗셈보다 길어집니다. 정확한 수치는 환경별 **실측이 필요**합니다(콜드스타트 최적화는 [① 06 6.5절 Cold Start](../../01-infra/docs/06-production.md#cold-start)로, 로딩 시간 1차 가정치는 [A1.3](../appendix/A1-first-order-reference.md#a13-모델-콜드스타트와-로딩-1차-가정치)으로 위임).

### 용량 오버헤드(가용 용량 환산)

성능과 별개로, vSAN ESA의 데이터 보호 정책은 **원시 용량 대비 가용 용량**에 직접 영향을 줍니다. VCF 9.1의 Auto-RAID는 3–5 호스트에서 FTT=1 RAID-5(2+1), 6 호스트 이상에서 FTT=2 RAID-6를 적용하며, 두 경우 모두 객체 기준 약 1.5배 용량 오버헤드가 발생합니다(압축과 중복제거 절감 이전 기준)([VCF Blog, "Auto-RAID in vSAN for VCF 9.1"](https://blogs.vmware.com/cloud-foundation/2026/05/08/auto-raid-in-vsan-for-vcf-9-1/); [Broadcom TechDocs, "Using RAID 5/6 Erasure Coding"](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/vsan-deployment-administration-and-monitoring/administering-vmware-vsan/increasing-space-efficiency-in-a-vsan-cluster/using-raid-5-6-erasure-coding-in-vsan-cluster.html)).

즉 **필요 가용 용량 ≈ (논리 데이터 합계) × 약 1.5 ÷ (압축과 중복제거 효과)** 로 1차 환산합니다. 압축과 중복제거 효과는 데이터 특성에 따라 달라지므로 보수적으로 1.0(절감 없음)으로 시작해 실측으로 보정하세요.
...
모델 가중치 계수는 본 문서 5.5절 워크시트의 입력 가정을 재사용합니다(예: 70B FP16 ≈ 1버전 140GB — [5.5절 입력 가정](#55-사이징-워크시트--규모에서-용량으로) 참조). NVIDIA 컨테이너 이미지의 구체 용량은 공식 문서에 고정값이 없으므로 본 문서에서 임의 가정치로 제시하지 않으며, 5.5절의 NIM 항목과 마찬가지로 실측으로 채웁니다.
...
| 항목 | 1차 산정 | 메모 |
|---|---|---|
| PAIS 플랫폼 아티팩트(Harbor 프로젝트) | 20GB | 공식 고정값(고정 하한), 모델과 NIM 미포함 |
| 모델 갤러리(가중치) | Σ(모델 가중치 × 보존 버전 수) | 5.5절 계수 재사용(예: 70B FP16 ≈ 140GB/버전) |
| NVIDIA 컨테이너 이미지(NIM/드라이버) | 별도 산정(실측) | 공식 고정값 없음, 임의 가정치 제시 안 함 |
...
| 내부(앱↔pgvector↔모델) | 검색과 프롬프트 트래픽 | NSX 분산 스위칭/DFW | DFW 정책 오버헤드 확인 필요 |
| GPU 간 RDMA 패브릭 | 범위 밖 | 전용 패브릭 | ① 인프라로 위임 |

> 멀티호스트 GPU 간 RDMA(GPUDirect/RoCE) 패브릭은 본 표에서 의도적으로 제외했습니다. 설계와 사이징은 [① 인프라](../../01-infra/README.md)를 참조하세요.
...
| 로그와 관측(90일) | 100GB(어림) | 보존 정책 의존 |
| PAIS 플랫폼 아티팩트(에어갭) | 20GB(고정 하한) | 내부 Harbor 프로젝트, 5.4절 참조 |
...
| 로그와 관측 | 약 100GB | 90일 보존 가정 |
| **논리 합계** | **약 1,050GB(≈1.05TB)** | 위 합산 |
| **필요 가용 용량** | **약 1.6TB** | 논리 합계 × 1.5(보호 오버헤드, 압축 절감 미반영) |

> **공식 권장 비율로 교차 점검** — Broadcom 설계 권장(RAG 벡터 DB 저장 용량은 원문의 5–10배, 5.1)을 적용하면 원문 200GB에 대해 약 1–2TB가 나옵니다. 위 구성요소 합산(문서 코퍼스 300GB + 벡터 인덱스 180GB)보다 크게 높습니다. 이런 차이는 벡터 수(1,000만 건)와 원문 용량(200GB)을 서로 독립적으로 가정했기 때문에 생깁니다. 두 값을 표본 실측으로 한 번에 맞추기 전까지는 **두 계산 중 큰 값을 예산 상한으로** 두고, 실측 후 좁힙니다.

> 위 합계는 단일 예시이며, 모델 규모, 보존 버전 수, 벡터 건수가 바뀌면 선형 이상으로 변합니다. 특히 모델 버전 보존 정책과 벡터 인덱스 오버헤드 가정이 총량을 좌우하므로, 이 두 항목을 가장 먼저 실측하고 확정하세요. 압축과 중복제거를 적용하면 가용 용량 요건은 줄어들 수 있으나(데이터 특성 의존), 본 워크시트는 보수적으로 절감을 반영하지 않았습니다.

---

## 5.6 검증과 실측 방법

본 문서의 모든 수치는 출처 기반 어림이므로, 운영 전 다음 절차로 실측과 검증하세요.

1. **모델 아티팩트 실측**: Harbor 레지스트리에서 대상 모델의 실제 정밀도와 샤딩 기준 디스크 점유와 보존 버전 수를 확인합니다. FP16 140GB(70B)는 출발점일 뿐이며 양자화 시 크게 줄어듭니다([Spheron, "GPU Memory Requirements for LLMs"](https://www.spheron.network/blog/gpu-memory-requirements-llm/)).
...
4. **vSAN 성능과 용량 검증**: vSAN ESA 성능 대시보드로 IOPS, 지연, 대역폭을 확인하고([Broadcom TechDocs, "vSAN ESA Performance Dashboard"](https://techdocs.broadcom.com/us/en/vmware-cis/aria/aria-operations/8-18/vmware-aria-operations-configuration-guide-8-18/predefined-dashboards-in-vrealize-operations-manager/performance-dashboards/vsan-esa-performance-dashboard.html)), Auto-RAID 정책별 실제 가용/원시 용량 비율을 확인합니다([VCF Blog, "Auto-RAID in vSAN for VCF 9.1"](https://blogs.vmware.com/cloud-foundation/2026/05/08/auto-raid-in-vsan-for-vcf-9-1/)).
5. **네트워크 처리량 검증**: 추론 트래픽은 워크로드 믹스(텍스트/멀티모달)로 대역폭을 실측하고, NSX Edge 게이트웨이 처리량이 흐름과 서비스 조합에서 예상치를 충족하는지 확인합니다([VCF Blog, "VCF 9.1 Network Scale and Performance"](https://blogs.vmware.com/cloud-foundation/2026/05/05/simplify-workload-connectivity-and-enhance-network-scale-and-performance-with-vcf-9-1/)).
...
> 컴퓨트(GPU/CPU/메모리) 사이징, 콜드스타트 운영 최적화, 비용 환산은 본 시리즈의 다른 문서를 참조하세요. 본 문서는 스토리지와 네트워크 용량에 한정합니다.
