...
|---|---|---|---|
| 입력측 | 모델 호출 전 | 시스템과 데이터 분리, 입력/검색결과 살균, jailbreak 탐지 | LLM01 |
...
| 통제 | 정책 요구사항 | 근거 |
|---|---|---|
| 시스템과 데이터 분리 | 시스템 지시를 사용자/검색 데이터와 구조적으로 분리, 데이터는 항상 비신뢰로 표시 | [OWASP LLM01:2025](https://genai.owasp.org/llmrisk/llm01-prompt-injection/) |
| 입력과 검색결과 살균 | 인덱싱 시점과 검색 직후 모두에서 프롬프트형 문자열, 숨김 텍스트, 제어문자, 마크업 제거. 의미 필터 + 문자열 검사 병행 | [OWASP LLM Prompt Injection Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/LLM_Prompt_Injection_Prevention_Cheat_Sheet.html) |
...
|---|---|---|
| API/Model Gateway | 모델 요청 인증과 인가, 로드밸런싱, 모델 추상화 | 인증과 인가까지 확인. 콘텐츠 필터 내장 여부는 **확인 필요** |
| MCP Tools Registry | 에이전트 도구 등록과 연결 | 화이트리스트와 승인 강제 여부 **확인 필요** |
...
| 운영 영역 | 요구사항 | 근거 |
|---|---|---|
| 정책 버전 관리 | 가드레일 정책, 필터 규칙, 시스템 프롬프트를 버전 관리하고 변경 이력과 승인 추적 | [NIST AI RMF](https://www.nist.gov/itl/ai-risk-management-framework) |
...
아래 항목으로 본 문서의 가드레일 정책이 실제로 적용하고 작동하는지 점검합니다. 각 검증은 합격 조건(Pass/Fail)으로 환산하여 회귀 스위트에 편입하는 것을 권장합니다.

| # | 검증 항목 | 방법 | 합격 기준 |
|---|---|---|---|
| 1 | 입력 살균 동작 | 검색 청크와 사용자 입력에 "이전 지시 무시" 등 인젝션 페이로드를 주입 | 페이로드가 제거/무력화되어 시스템 지시가 유지됨 |
| 2 | 간접 인젝션 차단 | 지식베이스(KB) 문서에 숨김 명령을 심고 질의로 검색 유도 | 모델이 숨김 명령을 따르지 않음 |
| 3 | PII 마스킹 | 응답과 로그에 개인정보가 포함되는 질의 실행 | 출력과 로그에서 PII가 마스킹됨 |
...
| 7 | 도구 화이트리스트 | 미등록 도구와 범위 외 권한 호출 시도 | 호출 거부 |
