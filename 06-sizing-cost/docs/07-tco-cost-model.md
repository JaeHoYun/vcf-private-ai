...
**중요(가격에 대한 입장):** 이 문서는 라이선스 단가, 서버/GPU 단가, 전력 단가 등 어떤 실제 금액도 제시하지 않습니다. 모든 단가는 "공식 견적과 구독 조건으로 확인"이 필요한 입력값이며, 워크시트의 단가 칸은 비워 둡니다. 벤더가 발표한 TCO 절감 수치는 "벤더 발표(up to), 보수적 해석, 실측 필요"로만 인용합니다.
...
| 환율과 세금 | 분석 시점 고정값 | 실제 단가 확정 시 견적의 통화와 부가세 기준을 따릅니다(확인 필요). |
...
| --- | --- | --- | --- |
| GPU | GPU 모델과 수량 | 단가 × 수량 ÷ 상각연수 | (견적 확인) |
| GPU 서버(노드) | 노드 수, 폼팩터 | 단가 × 노드 수 ÷ 상각연수 | (견적 확인) |
| 메모리와 CPU | 노드별 사양 | 서버 단가에 포함 또는 옵션 | (견적 확인) |
...
| --- | --- | --- | --- |
| 인력(운영과 플랫폼) | 운영 FTE(전임 환산 인력) 수 | 인건비 × FTE | (산정 입력) |
| 전력 | 노드와 GPU 소비전력(kW) | kW × 가동시간 × 전력단가 × PUE(전력사용효율) | (사이트 데이터 확인) |
| 상면/냉각 | 랙 수, 열밀도 | 랙과 상면 단가 | (사이트 데이터 확인) |
| 유지보수/지원 | 하드웨어와 소프트웨어 계약 | 계약 단가(구독에 포함 여부 확인) | (견적 확인) |
...
비용 "추정"은 실제 청구와 사용량으로 반드시 보정해야 합니다. 아래는 추정 모델을 실측으로 검증하는 체크리스트입니다.
