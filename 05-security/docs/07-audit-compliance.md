...
| 드리프트(Drift) | 입력 분포와 품질 점수의 시계열 변화 | 모델과 데이터 분포 이탈 | LLM04 Data and Model Poisoning |
| 오남용(Abuse) | 비정상 호출 빈도, 거부율 급증, 시스템 프롬프트 추출 시도 | 인젝션과 프롬프트 유출 | LLM01, LLM07 |
...
7.2.2까지는 플랫폼 위에서 도는 AI의 관측입니다. 그런데 관측 대상에 오르지 않은 사용 — 개인 계정의 공개 챗봇, AI 기능이 켜진 SaaS, 개인 결제 API 키, 부서가 자체로 붙인 MCP 서버와 에이전트 — 는 위 표 어디에도 나타나지 않습니다. AX 방법론은 이를 섀도 AI(shadow AI, 조직이 존재를 모르는 채 쓰이는 AI)로 정의하고 등록, 등급 판정, 등급별 처분으로 이어지는 양성화 순서를 정했습니다([AX 방법론 07 7.6절](https://github.com/JaeHoYun/enterprise-ax-methodology/blob/main/docs/07-organization-and-control.md)). 이 절은 무엇이 어디에 숨는지와 그것을 찾는 세 채널을 정리하고, 그중 기술 관측 채널을 이 플랫폼이 이미 갖고 있는 신호로 구체화합니다.

**흔한 다섯 출처** — 과거의 섀도 IT와 다른 점은 데이터가 새어 나가는 것이 아니라 사람이 요약, 번역, 디버깅을 시키려는 선의로 데이터를 직접 붙여 넣는다는 것입니다.

| 출처 | 모습 | 주된 위험 |
|---|---|---|
| 개인 계정의 공개 챗봇 | 회사 자료를 넣어 요약, 번역, 초안 작성 | 입력이 사업자의 로그와 학습에 남음, 개인정보와 영업비밀 유출 |
| AI 기능이 내장된 SaaS | 노트, 회의록, 브라우저 확장, 협업 도구가 AI 기능을 켜면서 데이터를 외부 모델로 보냄 | 조직이 AI를 도입한 적 없다고 믿는 채 데이터가 흐름 |
| 개인 결제 API 키와 코드 어시스턴트 | 개발자가 개인 카드로 받은 키를 코드에 넣거나 개인 계정으로 코드 어시스턴트를 씀 | 소스코드 유출, 저장소에 남는 키, 개인 정산에 숨는 비용 |
| 부서가 자체 구축한 에이전트와 MCP 연결 | 부서 예산으로 만든 에이전트가 사내 시스템과 데이터에 직접 연결 | 소유자 불명의 행위 주체, 과잉 권한, 도구 공급망 오염([08 8.5절](08-agent-governance.md)) |
| 종료하지 못한 파일럿 | 끝났어야 할 파일럿이 퇴역 절차 없이 계속 돎 | 권한과 데이터 연결이 살아 있는 유령 자산 |

**발견의 세 채널** — 한 채널로는 면적이 잡히지 않으므로 셋을 겹칩니다.

| 채널 | 무엇을 보나 | 잡는 것 | 못 잡는 것 |
|---|---|---|---|
| 기술 관측 | 아래 신호 표 | 사내 네트워크와 관리 기기에서의 사용 | 개인 기기, 개인 망, 종단 암호화된 앱 |
| 재무 기록 | 법인카드와 경비 청구의 AI 서비스 결제, 부서 단위 SaaS 구독, 클라우드 계정의 AI API 과금 | 유료 도구와 부서 자체 구축 | 무료 요금제, 개인 결제 |
| 사람 | 면책 등록 기간(일정 기간 안에 신고하면 불이익 없이 등록해 주는 제도), 부서별 자가 신고, 현업 인터뷰 | 왜 쓰는지와 무엇을 넣는지라는 맥락 | 신고할 유인이 없거나 처벌이 예상되면 침묵 |

재무 기록과 사람 채널은 플랫폼 밖에서 운영하므로 보안팀 혼자 돌릴 수 없고, 재무 조직과 현업 부서장의 협조가 전제입니다. 사람 채널은 신고가 곧 징계가 아닐 때만 작동합니다. 세 채널의 결과는 인벤토리 한 곳에 모읍니다. 양식은 [거버넌스와 데이터주권 갭 워크시트](../worksheet/governance-sovereignty-gap.md) A절과 A-1(발견 채널 점검)입니다. 아래는 기술 관측 채널의 신호입니다.

| 신호 | 어디서 잡나 | 후보로 가르는 기준 | 한계 |
|---|---|---|---|
| 외부 AI 서비스 도메인으로의 이그레스 | vDefend Gateway Firewall의 URL 필터링 로그와 DNS 질의 기록([02 2.4절](02-network-tenant-isolation.md)) | 승인된 원격 모델 경로([05 5.6절](05-data-governance.md)의 반출 통제)에 없는 AI 서비스 도메인으로의 연결 | 개인 기기와 개인 망은 보이지 않음. 도메인 목록은 주기 갱신이 필요 |
| 이그레스 허용 목록 위반 | NSX VPC와 Antrea 정책의 거부 로그([02 2.2절, 2.3절](02-network-tenant-isolation.md)) | 허용 목록 밖 목적지로의 반복 시도, 특히 워크로드 네임스페이스 발신 | 허용 목록이 넓으면 신호가 사라짐 |
| 미승인 SaaS의 OAuth 동의 | IdP의 앱 동의 기록([03 3.1절](03-identity-access.md)) | 승인 목록에 없는 앱이 메일, 문서, 캘린더 범위의 동의를 받음 | IdP 밖의 개인 계정은 보이지 않음 |
| 저장소에 남은 외부 AI API 키 | 코드 저장소와 CI의 시크릿 스캔(키 관리 원칙은 [03 3.5절](03-identity-access.md)) | 외부 AI 사업자의 키 패턴 검출, 커밋 이력 포함 | 로컬에만 있는 키는 못 잡음 |
| 미등록 에이전트와 MCP 서버 | PAIS 네임스페이스의 에이전트 목록과 Tool Gallery를 레지스트리와 대조([08 8.4절](08-agent-governance.md)), 승인 목록 밖 MCP 엔드포인트로의 연결([03 3.4절](03-identity-access.md)) | 레지스트리에 없는 에이전트, Tool Gallery 밖의 도구 서버 | 플랫폼 밖에서 돌리는 에이전트는 이그레스 신호로만 잡힘 |
| 플랫폼이 집계하지 않는 과금 | 원격 모델 경로의 토큰 사용량 집계([① 06 6.8절](../../01-infra/docs/06-production.md))와 클라우드 계정의 AI API 과금 대조 | 플랫폼 집계에 없는 과금이 있으면 플랫폼 밖 호출 | 개인 결제는 재무 채널의 몫 |

탐지된 후보의 첫 조치는 차단이 아니라 등록입니다. 인벤토리 초판([00 0.3절](00-where-to-start.md)의 1–30일)과 에이전트 레지스트리([08 8.4절](08-agent-governance.md))에 올리고, 위험 등급을 판정한 뒤 고위험에 한해 대체 경로(승인된 원격 모델 경로 또는 로컬 모델)를 연 다음 차단합니다. 대체재 없는 차단은 사용을 개인 기기로 밀어내 관측 밖으로 보낼 뿐입니다. 재발을 막는 장치는 새로 만들지 않고 이미 있는 것에 겁니다. 위 신호를 일회성 조사가 아니라 상시 신호로 두고, 등록과 퇴역을 레지스트리의 기본 동작으로 만들며, 실측 자산 중 미등재 비율과 발견에서 양성화까지의 리드타임과 공식 도구의 채택률을 함께 봅니다. 미등재 비율이 줄지 않는데 공식 도구의 채택률도 낮다면 문제는 통제가 아니라 공급입니다. Broadcom이 예고한 vDefend와 Avi Load Balancer의 섀도 AI 탐지 기능은 버전과 시점이 공개되지 않았으므로([① 00 0.7절](../../01-infra/docs/00-whats-new.md)), 지금은 위 표의 현행 통제로 설계합니다.

---

## 7.3 사고 대응 플레이북 개요 (AI 특유의 사고)

AI 플랫폼 사고 대응은 NIST CSF 2.0의 운영 기능 Detect → Respond → Recover 흐름을 따르며, 그 위에 전략과 책임 구조를 정의하는 Govern 기능이 가로지릅니다([NIST CSF 2.0 — 6 Functions](https://csf.tools/reference/nist-cybersecurity-framework/v2-0/)). 아래는 AI 특유의 사고 세 유형의 플레이북 개요입니다.
...
VCF 9.1의 통합 사이버 복구(Integrated Cyber Recovery)는 온프레미스 격리 클린룸으로 복구하며, 내장 AI/ML 기반 EDR로 복원 지점(restore point)을 검증해 파일과 파일리스 악성코드를 식별함으로써 복구 데이터가 운영에 닿기 전에 깨끗함을 검증합니다([VMware — Continuous Compliance & Cyber Recovery for VCF 9.1](https://blogs.vmware.com/cloud-foundation/2026/05/05/continuous-compliance-integrated-cyber-recovery-and-enhanced-platform-security-for-vcf-9-1/)). 사고 조사 단계에서는 7.1.3의 Audit Trail이 표준화된 로그와 중앙 이력을 제공해 "무엇이, 왜 일어났는가"를 재구성하는 포렌식 근거가 됩니다([Broadcom — VCF 9.1 발표](https://news.broadcom.com/releases/broadcom-announces-vmware-cloud-foundation-9-1)).
...
한국의 법령과 지침이 플랫폼 설계에 직접 요구하는 것을 이 시리즈의 통제에 대응시킵니다. 요구의 원문과 시행 일정은 각 기관의 공표문과 [AX 방법론 부록 A2](https://github.com/JaeHoYun/enterprise-ax-methodology/blob/main/appendix/A2-kr-regulatory-timeline.md)로 확인하고, 해당 여부는 법무 검토로 확정합니다. 이 표는 통제의 위치를 찾는 색인이지 법적 판단이 아닙니다.
...
| 국가정보원 국가 공공기관 AI 보안 가이드북(2025-12-10) | 허용 목록 기반 도구 체계, 에이전트에 사람의 개입, 체크리스트 점검 | [03 3.4절](./03-identity-access.md) 승인 게이트, [08 8.5절, 8.7절](./08-agent-governance.md), 7.5 체크리스트 |
| 과기정통부와 KISA AI 보안 안내서(2025-12-10) | API 통신 암호화와 호출 제한, 모델 드리프트 탐지 | [03 3.3절](./03-identity-access.md), [02](./02-network-tenant-isolation.md), 7.2 |
| 전자금융감독규정 시행세칙 개정(2026-04-20 의결) | 내부망에서 쓰는 외부 서비스의 평가와 정기 점검 조건 | 온프레미스 Private AI는 내부망 안에서 완결되므로 외부 서비스 예외 절차의 대상이 아니며, 원격 클라우드 모델을 붙이는 순간 그 경로만 대상이 됩니다([05 5.6절](./05-data-governance.md)) |

이 표가 법령과 통제의 대응을 보여 준다면, 운영 중인 자산 하나하나가 그 의무를 어디까지 채웠는지를 실측하는 양식은 [거버넌스와 데이터주권 갭 워크시트](../worksheet/governance-sovereignty-gap.md)입니다. B-0절이 인공지능기본법의 의무를 자산별로 점검하고, B-1–B-3절이 NIST AI RMF, EU AI Act 부속서 IV, ISO/IEC 42001에 대고 격차를 비추며, C절이 규제 데이터의 외부 경유를 소급해 추적합니다.

---

## 7.5 마스터 통제 체크리스트 + 검증 절차

이 표는 01–06과 08 각 영역의 핵심 통제를 한 표에 모으고, 각 통제의 "검증 방법(동작을 어떻게 확인하는가)"을 명시합니다. 이 문서가 시리즈 ⑤의 검증 절차 총괄입니다. 검증은 "구성 확인(설정이 있는가)"에 머물지 않고, "동작 확인(실제로 막히는가/기록되는가)"까지 이어져야 합니다. "게이트" 열은 그 통제가 필수가 되는 가장 이른 단계입니다. PoC는 합성 데이터나 공개 데이터로 사내 소수가 써 보는 단계, 파일럿은 실제 데이터를 한정된 사용자가 읽기 전용으로 쓰는 단계, 프로덕션은 그 제한을 푸는 단계이며, 게이트 관점의 요약은 [00 0.4절](./00-where-to-start.md)에 있습니다.

| # | 영역(출처 문서) | 핵심 통제 | 검증 방법(동작 확인) | 합격 기준 | 게이트 |
|---|---|---|---|---|---|
| C-01 | [01 위협 모델](./01-threat-model.md) | 위협 모델과 자산 목록 최신화 | 최신 자산/데이터 흐름이 위협 모델에 반영됐는지 문서 대조 | 신규 자산 누락 0건 | 파일럿 |
| C-02 | [03 ID, 인증, 접근통제](./03-identity-access.md) | SSO, IdP 인증 강제 | 우회 로그인 시도 → 차단 + 인증 실패 로그 생성 확인 | 우회 차단 + 로그 남음 | PoC |
| C-03 | [03 ID, 인증, 접근통제](./03-identity-access.md) | 최소권한 RBAC | 권한 외 리소스 접근 시도 → 거부 + 인가 거부 로그 | 거부 + 로그 남음 | PoC |
| C-04 | [02 네트워크, 테넌트, GPU 격리](./02-network-tenant-isolation.md) | GPU 워크로드 도메인 네트워크 분리 | 허용되지 않은 동서(east-west) 트래픽 시도 → 차단 확인 | 비인가 트래픽 차단 | 파일럿 |
| C-05 | [02 네트워크, 테넌트, GPU 격리](./02-network-tenant-isolation.md) | 정책 드리프트 연속 점검 | 의도적 정책 변경 주입 → 구성 준수 상시 점검 도구(ACC)가 드리프트 탐지와 교정 | 드리프트 탐지+자동 교정 | 프로덕션 |
| C-06 | [04 에어갭, 공급망, 모델 출처](./04-airgap-supply-chain.md) | 모델과 데이터 무결성 검증 | 변조 아티팩트 배포 시도 → 서명과 검증 단계에서 차단 | 변조본 거부 | PoC |
| C-07 | [05 데이터 거버넌스와 프라이버시](./05-data-governance.md) | 출력 민감정보 마스킹 | 민감정보 유도 프롬프트 → 출력 측 차단/마스킹 확인 | 노출 0건 | 파일럿 |
| C-08 | [06 앱 계층 가드레일](./06-app-guardrails.md) | 도구 호출 인가와 최소권한 | 비인가 도구 호출 시도 → 거부 + 도구 호출 로그 | 거부 + 로그 남음 | 파일럿(도구가 있으면) |
| C-09 | [06 앱 계층 가드레일](./06-app-guardrails.md) | 인젝션 가드레일 | 직접/간접 인젝션 페이로드 → 가드레일 차단 + 경보 | 차단 + 경보 발생 | 파일럿 |
| C-10 | [05 데이터 거버넌스와 프라이버시](./05-data-governance.md) | 정책과 책임 체계 운영 | 정책 문서와 역할표가 최신이고 실제 통제와 일치하는지 검토 | 정책-통제 불일치 0건 | 파일럿 |
| C-11 | 7.1 감사와 로깅 | 추적성(누가, 언제, 무엇을) | 임의 요청 1건을 trace ID로 인증→호출→도구→데이터까지 재구성 | 추적 체인 무결 | PoC |
| C-12 | 7.1.3 로그 무결성 | 변경 불가, 전송 암호화 | 로그 수정/삭제 시도 → 차단, 전송 TLS 적용 확인 | 수정 차단 + TLS 적용 | 파일럿 |
| C-13 | 7.2 관측과 드리프트 | 품질, 드리프트, 오남용 경보 | 합성 드리프트/이상 호출 주입 → Grafana 경보 발생 확인 | 경보 임계 내 발생 | 프로덕션 |
| C-14 | 7.3 사고 대응 | 플레이북 실행 가능성 | 테이블탑 훈련으로 탐지→대응→복구 절차 시연 | 단계 누락 없이 완주 | 프로덕션 |
| C-15 | 7.3.2 복구 | 클린룸 복구 검증 | 복원 지점 EDR 검증 후 격리 클린룸 복구 시연 | 악성 미검출 복원본 사용 | 프로덕션 |
| C-16 | [05 데이터 거버넌스와 프라이버시](./05-data-governance.md) 5.9절 | 보호 문서 복호화 범위와 회수 전파 | 복호화 로그를 승인 대장과 대조, 권한 회수 뒤 청크 비활성화까지 측정 | 승인 범위 밖 0건 + 목표 시간 내 비활성화 | 파일럿(보호 문서를 들이면) |
| C-17 | [08 에이전트 보안 거버넌스](./08-agent-governance.md) 8.3절, 8.4절 | 에이전트 레지스트리와 자율성 상한 집행 | PAIS 에이전트 목록과 레지스트리 대조, L1 에이전트에 승인 없는 쓰기 유도 | 미등록과 소유자 공란 0건 + 승인 없는 실행 0건 | 파일럿 |
| C-18 | [08 에이전트 보안 거버넌스](./08-agent-governance.md) 8.5절 | MCP 도구 공급망(버전 고정, 설명 해시, 재승인) | 승인된 도구의 설명 변경과 미승인 레지스트리의 서버 등록 시도 | 승인 자동 무효화 + 등록 차단 | 파일럿(외부 MCP 서버를 쓰면) |

> 검증 운영 권고: 각 통제는 (1) 책임자, (2) 검증 주기, (3) 최근 검증 결과/증적 링크를 함께 관리해, F(사실)→Claim(주장)→통제→검증 증적의 추적 체인이 끊기지 않도록 합니다. 검증 증적(스크린샷, 로그 export, 훈련 기록)은 7.1.3의 보존 정책에 따라 보관합니다. 게이트에서 면제한 항목은 사유와 만료일을 함께 적습니다.
...
[← 이전: 06 앱 계층 가드레일](06-app-guardrails.md) | [목차](../README.md) | [다음: 08 에이전트 보안 거버넌스 →](08-agent-governance.md)
