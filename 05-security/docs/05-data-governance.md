...
데이터 거버넌스는 누가 무엇을 책임지는지부터 정해야 통제가 끊기지 않습니다. 프라이빗 AI 플랫폼에서는 인프라 소유자(VCF/GPU WLD 운영), 데이터 소유자(원본 시스템과 KB 관리), 플랫폼 운영자(파이프라인과 벡터DB), 애플리케이션 소유자(앱 가드레일)의 책임이 명확히 분리되어야 합니다.

| 역할 | 책임 범위 | 본 문서 관련 통제 |
|---|---|---|
| 인프라 소유자 | GPU WLD, 격리, 데이터 상주(residency) 경계 | 5.6 |
| 데이터 소유자 | 원본 ACL, 접근등급 부여, 보존과 삭제 정책 | 5.2, 5.5 |
...
| `protection_policy_id` | 원본 문서보안(DRM) 정책 식별자(보호 문서만, 5.9절) | `drm-pol-fin-02` |
| `decrypted_at` | 복호화 시각(보호 문서만) | `2026-09-10T02:00Z` |
| `decryption_approval` | 복호화 승인 근거(승인 대장 항목 ID) | `apr-2026-031` |
...
| 단계 | 위치 | 역할 | 실패 시 영향 |
|---|---|---|---|
| 사전 필터 | 벡터 검색 쿼리(pgvector WHERE 절) | 권한 없는 청크 후보 배제, 성능과 정확도 확보 | 권한 외 문서가 후보에 진입 |
...
| 마스킹/편집(redaction) | 비정형 텍스트의 PII를 토큰으로 치환 | 본문에 섞인 이름과 연락처 |
| 가명화(pseudonymization) | 식별자를 가역 토큰으로 대체 | 추후 재식별 필요한 분석 |
| 익명화(anonymization) | 비가역적으로 식별 불가 처리 | 장기 보존과 재식별 불요 |
...
- 외부 연동이 필요한 경우 국외 이전 여부를 정책으로 명시하고, 이전 경로와 로그를 추적 가능하게 남깁니다(append-only 로깅 권장).
- 역할과 지역 기준 RBAC로 누가 어느 경계의 데이터를 다룰 수 있는지 제한합니다([Cross-Border Data Residency (Airbyte)](https://airbyte.com/data-engineering-resources/cross-border-data-residency)).
...
여러 부서와 고객이 한 플랫폼을 공유할 때, 테넌트 간 데이터가 섞이지 않아야 합니다. OWASP LLM08은 교차 테넌트 누출(cross-tenant leakage)을 벡터와 임베딩 취약점의 대표 사례로 꼽습니다([OWASP LLM08](https://www.indusface.com/learning/owasp-llm-vector-and-embedding-weaknesses/)).

| 격리 수준 | 방식 | 트레이드오프 |
|---|---|---|
| 강한 격리 | 테넌트별 별도 벡터DB 인스턴스/스키마 | 자원과 운영 비용 증가, 누출 위험 최소 |
...
|---|---|---|
| 검색단 권한 필터 | 권한 메타데이터, 재동기화, 필터 정책 정의 | 사용자 인증 컨텍스트 검증과 전달 |
| PII | 인입 마스킹, 분류와 플래그 부여 | 출력 응답 검열과 차단 런타임 |
...
| # | 검증 항목 | 방법 | 합격 기준 |
|---|---|---|---|
| 1 | 분류와 메타데이터 완전성 | 표본 청크의 `classification`, `acl_principals`, `pii_flag` 존재 확인 | 필수 필드 누락 0건 |
...
| 7 | 임베딩 잔존 | 삭제 대상 문서의 임베딩 복원과 멤버십 추론 시도 | 색인에서 미검출 |
...
| 11 | 보호 문서 복호화 범위 | 복호화 로그의 문서 목록을 승인 대장의 범위(등급, 폴더, 기간)와 대조 | 승인 범위 밖 복호화 0건 |
| 12 | 보호 문서 회수 전파 | 문서보안 서버에서 권한 회수 또는 등급 상향 뒤 청크 비활성화까지 측정 | 정의된 목표 시간 내 비활성화 |
| 13 | 대화 데이터 보존과 삭제 | 표본 세션의 로그, 피드백, 추적 데이터, 캐시가 보존 정책대로 만료되고 사용자 삭제 요청이 전 계층에 전파되는지 확인 | 만료 초과 잔존 0건, 삭제 요청 전파 누락 0건 |

검증은 1회성이 아니라 정기 회귀로 운영하며, 통제 변경(스키마, 필터, 동기화 주기) 시 재실행합니다. 검증 결과는 5.1 책임 모델의 데이터 소유자와 플랫폼 운영자에게 보고되어 시정 조치로 연결되어야 합니다.
...
- ② 벡터DB 가이드: [② 벡터 DB](../../02-vectordb/README.md) — pgvector 스키마와 DSM 운영
- ④ RAG 참조 아키텍처: [④ RAG](../../04-rag/README.md) — 검색 파이프라인과 인프라 격리
- [06 앱 가드레일](06-app-guardrails.md): 런타임 입력과 출력 통제(본 시리즈)
